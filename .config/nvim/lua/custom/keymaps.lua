...
vim.keymap.set('n', '<leader>S', function()
  -- Find existing scratch buffer or create new one
  local scratch_bufnr = nil
  for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
    if vim.api.nvim_buf_get_option(bufnr, 'buftype') == 'nofile' and vim.api.nvim_buf_get_name(bufnr):match 'scratch' then
      scratch_bufnr = bufnr
      break
    end
  end

  if scratch_bufnr and vim.api.nvim_buf_is_loaded(scratch_bufnr) then
    -- If scratch buffer exists and is visible, close it
  ...
  local win = vim.api.nvim_open_win(scratch_bufnr, true, {
    relative = 'editor',
    width = width,
    height = height,
    col = col,
  ...
  -- Close with q or <Esc>
  vim.keymap.set('n', 'q', '<cmd>close<cr>', { buffer = scratch_bufnr, silent = true })
  vim.keymap.set('n', '<Esc>', '<cmd>close<cr>', { buffer = scratch_bufnr, silent = true })
